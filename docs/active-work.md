...
- Add an item here when it becomes active, paused, or blocked.
- Keep detailed notes in `docs/work-items/`, not in this index.
- Update the item label and status when the current focus changes.
- Remove completed items from this active list only after their final validation and durable handoff are recorded in the work-item file.
- Continue using `docs/interactive-grade3/task-tracker.md` for detailed Eureka Math implementation history and validation evidence.
