...
Status: Active — Reading year map and 100 official text-evidence questions verified; complete daily lessons and official assessments remain blocked on edition-matched materials
...
- A second publisher-authored source now admits 100 official Grade 3 Text Evidence Questions, ten per unit. The active Unit pages organize them by their verified week and selection, and the question routes require the school text while providing a clearly labeled portal-authored Learn → Watch → Practice → Respond teaching layer.
...
- Every active Unit page now leads with three child-readable week learning paths—Read, Learn, and Practice—using only source-controlled selections, instructional focuses, word study, writing, and official questions. The complete scope and question page images remain visibly available in the same Unit page as the evidence layer.
...
Treat the verified 10-unit, 30-week, 70-selection, 100-question Reading year; 20 rendered official source pages; 44-record California standards map; sourced assessment/reading-level system; quarantined older practice registry; and sealed Math behavior as the current desktop baseline. Complete daily lesson expansion still begins only when authorized Benchmark student and Teacher Resource System pages are available.
...
- Admitted the district-hosted publisher file `Grade 3 Text Evidence Questions for Texts for Close Reading` after fingerprinting and visually reviewing all ten pages. Mapped all 100 official questions to the verified unit, week, selection role, and title register.
- Replaced the Unit source-ledger/text-dump layout with a learner path modeled on the Math experience: each week now presents the exact selections, exact published learning focuses, source-controlled evidence-question launches, word study, and writing in a clear Read → Learn → Practice hierarchy.
- Added 100 interactive evidence-practice routes with a school-book gate, official question, strategy-specific progressive model, tap-first guided practice, spoken response, tap-to-confirm self-checks, direct official source evidence, and previous/next navigation. No passage, answer, score, classroom pacing, or learner text-entry form is supplied or inferred.
- Replaced the worksheet-like multi-textarea response path with a Reading-native staged studio. Twelve independent original supplemental demonstrations teach the reading moves without using Benchmark passage facts or implying official answers.
- Separated authority explicitly: official sources control curriculum content, the IES/REL K–3 comprehension progression controls the teaching sequence, and established elementary literacy products inform interaction mechanics only.
...
3. Obtain authorized Grade 3 Benchmark student selection pages and Teacher Resource System lesson pages before expanding complete daily lessons beyond the single admitted `u1-w1-l1` pilot. The current 100-question path may be used only with the learner’s school texts.
...
- For the next increment, begin from the verified ten-unit Reading registry, 10 scope images, 10 question images, 100 official questions, the quarantined older practice registry, and the protected Math baseline. Do not replace the publisher unit spine with generic standards areas, a small text library, or global prompt phases.
...
| 2026-08-13 | Bounded official Reading lesson pilot | Superseded by keyboard-free remediation | Admitted Benchmark Education's official Grade 3 Unit 1 Week 1 sample and released only Lesson 1 “Working Together.” The initial digital response fields were later rejected because they reproduced worksheet behavior. The current lesson retains the admitted source boundary but uses tap, reveal, locate-in-book, and speak interactions instead. |
...
| 2026-08-13 | Official 100-question Reading teaching layer | Passed | Fingerprinted and visually reconciled the ten-page Benchmark text-evidence source; validated 10 units, 30 weeks, 70 selections, 100 questions, 20 rendered evidence pages, 44 California records, and the single bounded teacher-sample lesson. Live Gemini-profile Chrome review clicked the portal, Reading year, Unit 1, Question 1, Question 10, all four information areas, portal return, Math home, and a deep Math Solved route. An answer carry-over defect was found, fixed, and revalidated. Automated browser coverage opened 117 portal/Reading routes with 0 failures, no horizontal overflow, and no console warnings/errors. All 19 visible external source URLs returned HTTP 200. Production build and the unchanged 7/35/152/673 Math search contract passed. |
| 2026-08-13 | Reading-native tap-first teaching studio | Superseded by keyboard-free remediation | Replaced the multi-textarea response surface with Learn → Watch → Practice → Respond, but still retained one optional typed answer. That remaining worksheet behavior was rejected and removed in the keyboard-free increment below. |
| 2026-08-13 | Keyboard-free Grade 3 Reading interaction guard | Passed | Removed every learner-facing Reading input and textarea from both the 100-question practice route and the bounded official Lesson 1 pilot. The paths now use tap, choose, reveal, locate-in-book, speak, and tap-to-confirm interactions. The fail-closed Reading validator rejects form controls, Angular two-way form bindings, and removed typed-response labels. In the approved Gemini Chrome profile, all 100 practice routes rendered the no-typing spoken-response stage with zero visible learner form controls, overflow, or route failures; the complete Question 1 path was finished without a keyboard; all 20 Lesson 1 learner actions were completed; portal → Reading → Unit 1 → Question 1 navigation passed; and Math home plus a deep Module 3 Lesson 5 Blank/Solved route remained operational. Reading curriculum, source-readiness, Math search, production build, and diff checks passed. |
| 2026-08-13 | Reading source proof at point of use | Passed | Moved the real rendered Benchmark scope and question pages above each Unit learning path, added the exact publisher question-page link beside all 100 question launches, and placed the matching rendered question page beside every practice prompt. Gemini Chrome clicked Unit 1 → interactive guide → Question 2 and exercised the open-book, think-aloud, and guided-practice controls. A complete browser audit covered all 10 Unit routes and all 100 practice routes with 0 missing source previews, 0 missing exact source links, 0 Reading learner inputs/textareas, 0 horizontal overflow, and 0 route failures. Both publisher PDF endpoints returned HTTP 200 as `application/pdf`; Reading validators and production build passed; a deep Module 3 Lesson 5 Math Solved route retained its title, four problems, Blank/Solved controls, and visual model. |
...
2026-08-13 — Admitted the verified 100-question Benchmark text-evidence source, rebuilt each Unit as a source-controlled Read → Learn → Practice path, added strategy-specific interactive evidence workspaces, and passed complete route, source-link, visual, interaction, build, and Math non-regression checks.

2026-08-13 — Removed the remaining digital-worksheet behavior from Reading. Official question content remains source-controlled; 12 separately labeled original demonstrations provide progressive modeling; and the active learner path now uses tap, reveal, locate-in-book, speak, and tap-to-confirm interactions with no learner text entry. The validator fails if learner-facing inputs, textareas, two-way form bindings, or removed typed-response labels return.

2026-08-13 — Closed the source-visibility gap: both governing publisher pages now precede the Unit path, every question launch carries its exact publisher-page link, and the active question shows the real rendered publisher page beside the transcribed prompt. The validator now fails if source proof is buried, detached from a question launch, or absent from a practice prompt.
